/**
 * Created by tarena on 2016/11/11.
 */
...
function jumpToList(param){
    console.log("jumpToList:param:"+param);
    var url = "./resultList.html?kindName=";
    var kindName = param;
    window.open(encodeURI(url+kindName));
}
...
$(function(){
    ...
    $.ajax({
        url:"data/index_kind.php",
        success:function(data)
        {
            var html="";
            $.each(data,function(i,p)
            {    html+=`<li><a href="javascript:jumpToList('${p}')">${p}</a></li>`;
                //./resultList.html?kindName=${p}
            });
            $('#search>ul').html(html);        
        }
    });
  
  $("#txtBtn button").on("click",function(){
      var input = $("#txtBtn input").val();
