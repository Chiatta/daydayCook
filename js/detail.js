...
function jumpToListSearch(param){
  ...
  $("#totalRank").on('click',function(){
    $.ajax({
          url:"data/suggestFoodTotalRank.php?foodId="+id,
  ...
  $("#lookRank").on('click',function(){
    $.ajax({
          url:"data/suggestFoodLookRank.php?foodId="+id,
          success:function(data)
          {
              console.log(data);
              var html=``;
  ...
  $("#commentRank").on('click',function(){
    $.ajax({
          url:"data/suggestFoodCommentRank.php?foodId="+id,
          success:function(data)
          {
              console.log(data);
              var html=``;
   ...
   $("#loginSubmit").on("click",function (){
    ...
    $(".talkSubmit .commit").on("click",function(){
      // 判断评论框中是否为空，如果为空跳出方法不执行评论操作，如果不为空进行接下来的操作。
        var rankSel = $("#rankSel").val();
        var rankNumber = (rankSel == 1||rankSel == 2||rankSel == 3||rankSel == 4||rankSel == 5)?1:0;
        var commentContent = $(".talkBox textarea").val();
        if(commentContent && rankNumber){
            // 如果浏览器cookie中存在userId，即用户成功登录后，如果用户没有登录，弹出你还没有登录的提示信息
            if($.cookie("userId")){
              // 发起ajax请求，该请求是为了在评论表中插入该用户的评论信息
                $.ajax({
                  // 请求路径，进行拼接，传递该php文件所需参数。
                    url:"data/comment.php?foodID="+id+"&userId="+$.cookie("userId")+
                                  "&contents="+commentContent+"&rank="+rankSel,
                    success:function(data){
        ...
        }else if(commentContent && !rankNumber){
            alert("您还没有选择评分");
        }else{
            alert("您还没有输入评论");
        }
